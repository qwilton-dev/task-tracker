...
type mockActivityEventRepo struct{}

func (m *mockActivityEventRepo) Create(ctx context.Context, event *domain.ActivityEvent) error {
	return nil
}
func (m *mockActivityEventRepo) ListByIssue(ctx context.Context, issueID string) ([]*domain.ActivityEvent, error) {
	return nil, nil
}
...
func TestIssueService_CreateIssue(t *testing.T) {
	...
	svc := NewIssueService(NewActivityEventService(&mockActivityEventRepo{}), repo, projRepo, nil)
...
func TestIssueService_CreateIssue_EmptyTitle(t *testing.T) {
	svc := NewIssueService(NewActivityEventService(&mockActivityEventRepo{}), &mockIssueRepo{}, &mockProjectRepo{}, nil)
...
func TestIssueService_MoveIssue_InvalidStatus(t *testing.T) {
	svc := NewIssueService(NewActivityEventService(&mockActivityEventRepo{}), &mockIssueRepo{}, &mockProjectRepo{}, nil)
	err := svc.MoveIssue(context.Background(), "issue-1", "invalid", 0, "user-1")
	if err == nil {
...
func TestIssueService_MoveIssue_ValidStatus(t *testing.T) {
	...
	svc := NewIssueService(NewActivityEventService(&mockActivityEventRepo{}), repo, &mockProjectRepo{}, nil)
	err := svc.MoveIssue(context.Background(), "issue-1", "todo", 100, "user-1")
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
}
...
func TestIssueService_UpdateIssue_TitleTooLong(t *testing.T) {
	...
	_, err := svc.UpdateIssue(context.Background(), "issue-1", title, "", "", "", "user-1")
...
func TestIssueService_UpdateIssue_InvalidPriority(t *testing.T) {
	...
	svc := NewIssueService(NewActivityEventService(&mockActivityEventRepo{}), repo, &mockProjectRepo{}, nil)
	_, err := svc.UpdateIssue(context.Background(), "issue-1", "", "", "invalid", "", "user-1")
