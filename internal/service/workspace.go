...
func (s *WorkspaceService) CreateWorkspace(ctx context.Context, userID, name string) (*domain.Workspace, error) {
	ws, err := domain.NewWorkspace(name)
	if err != nil {
		return nil, err
	}
	...
	if err := s.repo.AddMember(ctx, ws.ID, userID, "owner"); err != nil {
		return nil, err
	}
	return ws, nil
}
...
func (s *WorkspaceService) UpdateWorkspace(ctx context.Context, id, name string) (*domain.Workspace, error) {
