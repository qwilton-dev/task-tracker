...
func (r *ProjectRepository) CreateProject(ctx context.Context, project *domain.Project) error {
	query := `
		INSERT INTO project (workspace_id, name, slug)
		VALUES ($1, $2, $3)
		RETURNING id, created_at
	`
	return r.db.QueryRow(ctx, query, project.WorkspaceID, project.Name, project.Slug).
...
func (r *ProjectRepository) GetProjectsByWorkspace(ctx context.Context, workspaceID string) ([]*domain.Project, error) {
	query := `SELECT id, workspace_id, name, slug, created_at FROM project WHERE workspace_id = $1`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.WorkspaceID, &p.Name, &p.Slug, &p.CreatedAt); err != nil {
			return nil, err
		}
